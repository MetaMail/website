...
export default function MailListItem({
    ...
    tip={'star'}/>*/}</div>
            <span className='pt-4 text-[#333333] omit w-105 pl-25' onClick={onClick} >
              <span className={isRead?'text-black text-opacity-60':''}>{from?.name && from.name.length > 0 ? from.name : from.address}</span>
            </span>
            <div className={isRead?'bg-opacity-0 flex self-center w-4 h-4 rounded-full bg-[#006AD4] mx-7 mt-4':'flex self-center w-4 h-4 rounded-full bg-[#006AD4] mx-7 mt-4'}/>
            <span className='pt-4 text-[#333333] omit w-120 ' onClick={onClick} ><span className={isRead?'text-black text-opacity-60':''}>{subject}</span></span>
