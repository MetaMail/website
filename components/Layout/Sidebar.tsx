...
export default function Sidebar() {
    ...
    const renderBadge = (type: FilterTypeEn) => {
        ...
        return (
            <p className="badge badge-sm rounded-2 btn-primary p-0 w-30 h-18 lh-18">
            ...
            if (!item.children || item.children.length <= 0) {
                return (
                    <li
                        ...
                        <a
                            className={`rounded-5 p-0 h-32 pl-12 pr-5 flex justify-between items-center  transition-colors duration-100  ${
                                filterType === Number(item.key) ? 'active rounded-4 font-[600]' : 'hover:bg-base-300'
                            }  dark:!bg-#E7E7E71A dark:hover:bg-[#E7E7E70F] dark:!bg-opacity-10 `}>
                            <div className="flex items-center gap-x-9">
                                ...
                                <p
                                    className={`align-middle leading-[18px] h-[16px] ${
                                        filterType === Number(item.key) ? 'leading-[18px]' : ''
                                    }`}>
                                    {item.title}
                                </p>
                            </div>
                ...
                return (
                    <li key={item.key}>
                        {/* More */}
                        <div
                            className={`rounded-5 flex items-center p-0 h-32 pl-12 pr-5  menu-dropdown-toggle menu-dropdown-show after:w-0 transition-colors duration-100 ${
                                filterType === Number(item.key) && item.childrenShow
                                    ? 'active rounded-4'
                                    : 'hover:bg-base-300 dark:hover:bg-[#E7E7E70F] dark:!bg-opacity-10 '
                            }`}
                            onClick={() => handleToggle(menus_Map, index, 'childrenShow')}>
                            ...
                            <div
                                className={`transition-all duration-100 ease-in-out transform ${
                                    item.childrenShow ? ' rotate-180  ' : 'rotate-0'
                                }`}>
                                {renderLogo(item.logo, filterType === Number(item.key))}
                            </div>
                        ...
                        <ul
                            className={`ml-0 pl-0 before:w-0 menu-dropdown ${
                                item.childrenShow ? 'menu-dropdown-show' : ''
                            }`}>
        ...
        <div className="w-203 px-12 flex flex-col justify-between font-poppins flex-shrink-0 text-[16px]">
            <div className="flex flex-col">
                <button className="flex items-center justify-center py-10">
                    ...
                    <p className="text-[27px] text-[#000] font-['PoppinsBold']  leading-[43px] h-36 dark:text-[#fff]">
                        MetaMail
                    </p>
                </button>
                <button
                    className="btn-primary flex items-center justify-center text-white h-45 rounded-9 gap-9"
                    onClick={handleClickNewMail}>
                    <Image src={write} alt="new_mail" className="w-18 h-auto" />
                    <span className="text-[16px] ">New Message</span>
                </button>
                <div>
                    <ul className="menu px-4  py-9 text-[16px]">{renderLi(menus_Map)}</ul>
                </div>
            </div>
        </div>
    );
