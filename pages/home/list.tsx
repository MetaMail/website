          ...
          <div className="h-14 flex gap-10">
            {selectList.length
              ? sixList.map((item, index) => {
                  return <Icon url={item.src} key={index} onClick={item.handler} className="w-13 h-auto self-center" />;
                })
              : null}
          </div>
        </div>
          ...
          list.map((item, index) => {
            ...
            return (
              ...
              <button
                key={index}
                className={
                  selectList.findIndex(i => i.message_id === item.message_id && i.mailbox === item.mailbox) >= 0
                    ? 'text-left select-bg'
                    : 'text-left'
                }>
                <MailListItem
